...
class integral_Goods(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        # 加载地址
        driver.get("http://t-mintegral9.helitong.cn/#/AccountLogin")
        time.sleep(5)
    ...
    def test_integralGoods(self):
        try:
            ...
            while True:
                ...
                try:
                    # 跳转到页面底部
                    driver.execute_script("var q=document.documentElement.scrollTop=1000000")
                    # driver.execute_script("window.scrollTo(0, document.body.scrollHeight);") 
                    time.sleep(3)
                    ...
                    if goods == False:
                        ...
                        # 判断元素是否置灰
                        if not driver.find_element(By.XPATH, "//div[@class='van-goods-action']/button").is_enabled():
                            # 获取商品名称
                            goods_name = driver.find_element(By.XPATH, "//div[@class='goods_name']/following::div[6]").text
                            one_goods_name = ''.join(goods_name)
                            print(f"下单失败：\t商品名称：{one_goods_name}")
                            time.sleep(3)

                            try:
                                # 打开excel
                                word_book = xlrd.open_workbook('E:/clone/output.xls', formatting_info=True)
                                ...
                                # 添加内容到指定工作表
                                new_sheet = new_work_book.get_sheet("03.17")
                                # 追加内容
                                n = "%03d" % (i + 45)
                                new_sheet.write(old_rows, 0, n, style)
                                new_sheet.write(old_rows, 2, one_goods_name, style)
                                new_sheet.write(old_rows, 3, "失败", style)
                                new_sheet.write(old_rows, 4, "商品已售罄", style)
                                # 保存
                                new_work_book.save('E:/clone/output.xls')
                            ...
                            try:
                                ...
                                if integral == False:
                                    ...
                                    time.sleep(1.5)
                                    # 获取商品名称
                                    goods_name = driver.find_element(By.XPATH,
                                                                     "//div[@class='goods_name']/following::div[6]").text
                                    two_goods_name = ''.join(goods_name)
                                    print(f"下单失败：\t商品名称：{two_goods_name}")
                                    time.sleep(3)
                                    ...
                                    try:
                                        ...
                                        n = "%03d" % (i + 45)
                                        new_sheet.write(old_rows, 0, n, style)
                                        new_sheet.write(old_rows, 2, two_goods_name, style)
                                        new_sheet.write(old_rows, 3, "失败", style)
                                        new_sheet.write(old_rows, 4, "商品已售罄", style)
                                        # 保存
                                        new_work_book.save('E:/clone/output.xls')
                                    ...
                                    # 输入密码
                                    driver.find_element(By.XPATH, "//input[@class='password-code-input']").send_keys("123456")
                                    time.sleep(1)
